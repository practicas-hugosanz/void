...
if ($webSearch) {
    ...
    if ($serperKey) {
        // Extraer el último mensaje del usuario para determinar si necesita búsqueda
        $lastUserMsg = '';
        ...
        // Detectar si la pregunta necesita información reciente/actual
        $needsSearch = needs_web_search($lastUserMsg);

        if ($needsSearch) {
            if ($doStream) {
                // Avisar al cliente que estamos buscando
                ...
                echo 'data: ' . json_encode(['status' => 'searching']) . "\n\n";
                flush();
            }

            $searchResults = serper_search($serperKey, $lastUserMsg);
            if ($searchResults) {
                // Inyectar resultados en el system prompt
            ...
            if ($doStream) {
                // Ya pusheamos headers arriba, continuar con stream directamente
 ...
 * Evita búsquedas para preguntas conceptuales, código, matemáticas, etc.
 */
function needs_web_search(string $text): bool {
    ...
    // Preguntas cortas sobre quién/qué/cuándo que no sean conceptuales
...
/**
 * Busca con Serper.dev y devuelve un resumen de los resultados.
 * Requiere variable de entorno SERPER_API_KEY.
 */
function serper_search(string $key, string $query): string {
    ...
    if (!empty($data['answerBox']['answer'])) {
        ...
        $output .= '**Respuesta directa:** ' . $data['answerBox']['snippet'] . "\n\n";
    }

    if (!empty($data['knowledgeGraph']['description'])) {
        $output .= '**Conocimiento:** ' . $data['knowledgeGraph']['description'] . "\n\n";
    }
